...
* **/XO****（Exclude Older）：****跳过较旧的文件****。如果目标文件夹里的文件比源文件新，或者两者时间一致，则直接跳过，不进行覆盖（最常用于增量备份）。**
* **/XN****（Exclude Newer）：****跳过较新的文件****。如果目标文件夹里的文件比源文件旧，则跳过不覆盖。**
* **/XC****（Exclude Changed）：****跳过已更改的文件****。跳过那些大小不同但时间戳相同的文件。**
* **/XN** + **/XO** + **/XC****：如果把这三个结合使用，Robocopy 将****绝对不覆盖任何目标端已存在的文件****，无论它们谁新谁旧。**
